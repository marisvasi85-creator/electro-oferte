...
export function CablePanel({
  ...
  const settingsOpen = showSettings || calibrating;
  ...
  const hasBom = calculation.billOfMaterials.length > 0;

  return (
    <section className="pe-cable-panel">
      <div className="pe-panel-title">
        <span>Deviz materiale</span>
        <small>Calcul automat din plan + catalog</small>
      </div>
      ...
      <div className="pe-cable-totals">
        ...
        <div>
          <strong>{calculation.counts.devicesTotal}</strong>
          <span>aparate pe plan</span>
        </div>
      </div>
      ...
      <div className="pe-export-group pe-bom-export">
        <button
          type="button"
          className="pe-cable-calibrate-btn pe-bom-primary"
          disabled={!hasBom || !onExportMaterialsCsv}
        ...
        <button
          type="button"
          className="pe-cable-calibrate-btn pe-bom-primary"
          disabled={!hasBom || !onExportMaterialsPdf}
      ...
      {!hasBom && (
        <p className="pe-muted pe-cable-hint">
          Plasează un tablou și aparate pe plan — aici apare lista de materiale și poți exporta devizul.
        </p>
      )}

      {!calculation.calibrated && (
        <p className="pe-cable-warn">Calibrează scara planului pentru metri reali de cablu.</p>
      )}
      ...
      <div className="pe-cable-breakdown">
        <div><span>Cablu 3x1,5</span><strong>{formatMeters(cable15?.lengthWithReserveM ?? 0)}</strong></div>
        <div><span>Cablu 3x2,5</span><strong>{formatMeters(cable25?.lengthWithReserveM ?? 0)}</strong></div>
      ...
      >
        {settingsOpen ? "Ascunde setări calcul" : "Setări calcul (scară, înălțimi)"}
      </button>
      ...
      {settingsOpen && (
        <>
          ...
          <label>
            Topologie trasee
            <select
              value={settings.routingMode ?? "circuit_tree"}
              onChange={(event) =>
                onChangeSettings({
